...
class VoteController extends ComController {
	public function index() {
		...
		if ($keyword != '') {
			if ($field == 'vote_name') {
				$where [$prefix . 'em_vote.vote_name'] = array (
						'like',
						'%' . $keyword . '%' 
				);
			}
			if ($field == 'village_name') {
				$where ['vg.village_name'] = array (
						'like',
						'%' . $keyword . '%' 
				);
			}
		...
		if ($currentVillage) {
			$where [$prefix . 'em_vote.village_id'] = array (
					'eq',
					$currentVillage 
			);
		}
	...
	public function relationVillage() {
		...
		$vote_id = $_POST ['voteid']; // 投票主表id
		if (isset ( $vote_id ) && isset ( $totalselect )) {
			// 之前的关联信息删除
			$m = M ( 'em_vote_village' );
			$list = $m->where ( 'vote_id=' . $vote_id )->delete ();
			if ($list !== false) {
				// 添加新的关联信息
				$m->startTrans ();
				$result = true;
	...
	// 查看关联小区
	public function getvillagetovotebyid() {
		$vote_id = $_POST ['vote_id']; // 投票主表id
		if (isset ( $vote_id )) {
			$list = M ( 'em_vote_village' )->join ( ' LEFT JOIN qw_em_village ON qw_em_village.VILLAGE_ID = qw_em_vote_village.village_id' )->field ( 'qw_em_village.VILLAGE_NAME,qw_em_vote_village.*' )->where ( 'vote_id=' . $vote_id )->select ();
			$this->ajaxReturn ( array (
					'status' => 1,
					'message' => $list 
			) );
	...
	public function update($ajax = '') {
		...
		if ($openVote == 'on') {
			$openVoteOn = M ( 'em_dictionary' )->field ( "dict_key,dict_value" )->where ( "dict_name = 'isOpenVote' and dict_key='公开'" )->find ();
			$data ['open_vote_result'] = $openVoteOn ['dict_value'];
		} else {
			$openVoteOff = M ( 'em_dictionary' )->field ( "dict_key,dict_value" )->where ( "dict_name = 'isOpenVote' and dict_key='不公开'" )->find ();
	...
	public function result() {
		...
		foreach ( $em_vote_paper as $k => $v ) {
			...
			$allCount = M ( 'em_vote_result' )->field ( "{$prefix}em_vote_result.*" )->where ( "{$prefix}em_vote_result.vote_paper_id = $votePaperId" )->count ();
			$em_vote_paper [$k] ['allCount'] = $allCount;
			
			$em_vote_paper_option = M ( 'em_vote_paper_option' )->field ( "{$prefix}em_vote_paper_option.vote_paper_option_id,{$prefix}em_vote_paper_option.option_titile,{$prefix}em_vote_paper_option.option_desc,
    			{$prefix}em_vote_paper_option.option_pic,sum(case when r.vote_result_id <> '' then 1 else 0 end) as count" )->join ( "left join {$prefix}em_vote_result r on (r.option_value = {$prefix}em_vote_paper_option.vote_paper_option_id 
				or (r.option_value LIKE CONCAT({$prefix}em_vote_paper_option.vote_paper_option_id,',%') or r.option_value LIKE CONCAT('%,',{$prefix}em_vote_paper_option.vote_paper_option_id)))" )->where ( "{$prefix}em_vote_paper_option.vote_paper_id=$votePaperId" )->group ( "{$prefix}em_vote_paper_option.vote_paper_id,{$prefix}em_vote_paper_option.vote_paper_option_id,{$prefix}em_vote_paper_option.option_titile,
    			{$prefix}em_vote_paper_option.option_desc,{$prefix}em_vote_paper_option.option_pic" )->select ();
	...
	public function resultDetail() {
		...
		if ($keyword != '') {
			if ($field == 'village_name') {
				$where [$prefix . 'em_vote_result.village'] = array (
						'eq',
						$keyword 
				);
			}
			if ($field == 'tel') {
				$where [$prefix . 'em_vote_result.tel'] = array (
						'like',
						$keyword . '%' 
				);
			}
		...
		if ($questionType == 3) {
			...
			$where ['p.vote_paper_option_id'] = array (
					'eq',
					$optionId 
			);
		...
		$count = M ( 'em_vote_result' )->field ( $field )->join ( $join )->where ( $where )->count ();
		$emVoteResult = M ( 'em_vote_result' )->field ( $field )->join ( $join )->where ( $where )->order ( $order )->select ();
